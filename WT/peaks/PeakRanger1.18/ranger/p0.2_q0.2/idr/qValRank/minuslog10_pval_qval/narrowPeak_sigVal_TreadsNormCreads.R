...
peakDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/peaks/PeakRanger1.18/ranger/p0.2_q0.2/"
outDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/peaks/PeakRanger1.18/ranger/p0.2_q0.2/idr/qValRank/minuslog10_pval_qval/"
...
for(i in c(1, 3, 8)) {
  ...
  peaks <- cbind(peaks[,1:6], peaks[,11]/peaks[,12], -log10(peaks[,8]), -log10(peaks[,9]), peaks[,10])
  colnames(peaks) <- c("chr", "start0based", "end", "name", "score", "strand",
                       "TreadsNormCreads", "pVal", "qVal", "summit0based")
  head(peaks)
  write.table(peaks, file = paste0(outDir, "WT_SPO11oligos_RPI", i, "_peaks_peakranger_ranger_p0.2_q0.2_TreadsNormCreads.narrowPeak"),
