...
############################################################################################################################################
# Plot SPO11-1, MNase and H3K4me3 heatmaps with rows (features; e.g., genes) ordered according to promoter coverage levels of each dataset #
...
plotDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_terminators500bp/plots/"
matDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_terminators500bp/matrices/"
rowOrderDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_terminators500bp/row_order/"
...
print(tss)
terminators <- ttsPlus(genes, upstream = -1, downstream = 500)
print("******terminators******")
print(terminators)
...
# function to create coverage matrices and heatmaps for terminators
...
geneHeatmap <- function(signal, target, x) {
  ...
  col_fun1 <- colorRamp2(quantile(mat1_smoothed, c(0.5, 0.95)), c("white", "red"))
  ht1 <- print(EnrichedHeatmap(mat1_smoothed, col = col_fun1, name = "Coverage",
                               column_title = paste0(orderNames[x], " at terminators"),
...
mclapply(seq_along(orderGrl), function(x) {
  geneHeatmap(orderGrl[[x]], terminators, x)
}, mc.cores = 3, mc.preschedule = F)
...
geneHeatmapOrder <- function(signal, target, h, i) {
  set.seed(7539)
  ...
  col_fun1 <- colorRamp2(quantile(mat2_smoothed, c(0.5, 0.95)), c("white", "red"))
  load(rowOrderFiles[[h]])
  pdf(plotFiles[[h]][[i]])
  ht1 <- print(EnrichedHeatmap(mat2_smoothed, row_order = row_order_ht1[[1]],
                               col = col_fun1, name = "Coverage",
                               column_title = paste0(libNames[i], " around genes, by ", orderNames[h], " at terminators"),
...
lapply(seq_along(orderNames), function(h) {
  mclapply(seq_along(grl), function(i) {
    geneHeatmapOrder(grl[[i]], genes, h, i)
  }, mc.cores = 3, mc.preschedule = F)
})
