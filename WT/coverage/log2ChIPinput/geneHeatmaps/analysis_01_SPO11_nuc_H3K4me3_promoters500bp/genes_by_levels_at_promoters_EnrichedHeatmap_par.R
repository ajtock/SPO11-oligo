...
plotDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_promoters500bp/plots/"
matDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_promoters500bp/matrices/"
rowOrderDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_promoters500bp/row_order/"
...
print(tss)
proms <- promoters(genes, upstream = 500, downstream = 0)
print("******promoters******")
print(proms)
...
# function to create coverage matrices and heatmaps for proms
...
geneHeatmap <- function(signal, target, x) {
  ...
  ht1 <- print(EnrichedHeatmap(mat1_smoothed, col = col_fun1, name = "Coverage",
                               column_title = paste0(orderNames[x], " at promoters"),
...
mclapply(seq_along(orderGrl), function(x) {
  geneHeatmap(orderGrl[[x]], proms, x)
}, mc.cores = 3, mc.preschedule = F)
...
geneHeatmapOrder <- function(signal, target, h, i) {
  ...
  ht1 <- print(EnrichedHeatmap(mat2_smoothed, row_order = row_order_ht1[[1]],
                               ...
                               column_title = paste0(libNames[i], " around genes, by ", orderNames[h], " at promoters"),
