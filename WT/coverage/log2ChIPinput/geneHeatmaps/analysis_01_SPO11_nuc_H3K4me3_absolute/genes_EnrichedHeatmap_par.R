...
plotDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_absolute/plots/"
matDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_absolute/matrices/"
rowOrderDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/geneHeatmaps/analysis_01_SPO11_nuc_H3K4me3_absolute/row_order/"
...
geneHeatmap <- function(signal, target, x) {
  ...
  mat1_smoothed <- normalizeToMatrix(signal, target, value_column = "coverage",
                                     extend = 0, mean_mode = "absolute", w = w, trim = trim,
...
geneHeatmapOrder <- function(signal, target, h, i) {
  ...
  mat2_smoothed <- normalizeToMatrix(signal, target, value_column = "coverage",
                                     extend = 2000, mean_mode = "absolute", w = w, trim = trim,
                                     empty_value = 0, smooth = TRUE,
...
plotFiles <- lapply(seq_along(orderNames), function(h) {
               lapply(seq_along(grl), function(i) {
                 paste0(plotDir, libNames[i], "_by_", orderNames[h],
                        "_norm_cov_genes_heatmap_trim0.00_0.01_smoothed_absolute.pdf")
               })
             })
