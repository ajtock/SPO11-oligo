...
# csmit -m 24G -c 24 "Rscript permTest_kss_SPO11_RPI34_RPI35_rangerPeaks_idr0.05_vs_others.R"
...
plotDir <- "/projects/ajt200/BAM_masters/SPO11-oligo/suvh456/peaks/PeakRanger1.18/ranger/p0.2_q0.2/idr/qValRank/minuslog10_pval_qval/RPI34_RPI35_profiles_rangerPeaks_idr0.05/genome_wide/regioneR/plots/"
...
save(ptPeaksOtherPerChrom,
     file = paste0(outDir,
                   "permTest_kss_SPO11_RPI34_RPI35_rangerPeaks_vs_others.RData"))
...
write.table(ptPeaksOtherPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_kss_SPO11_RPI34_RPI35_rangerPeaks_vs_others_DataFrame.txt"),
            sep = "\t", row.names = F)

# plot graphical summaries of results
for(i in 1:length(ptPeaksOtherPerChrom)) {
  pdf(file = paste0(plotDir, otherNames[i], "_permTest_nperm10000_kss_SPO11_RPI34_RPI35_rangerPeaks_perChrom.pdf"), width = 10, height = 7)
  plot(ptPeaksOtherPerChrom[[i]], main = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i]), xlab = "Number of overlaps", ylab = "Density")
  dev.off()
  ...
  pdf(file = paste0(plotDir, otherNames[i], "_localZscore_permTest_nperm10000_kss_SPO11_RPI34_RPI35_rangerPeaks_w1kb_s50bp_w10kb_s500bp_w", win ,"kb_s", step, "bp_perChrom.pdf"))
  par(mar=c(5.1, 4.1, 4.1, 2.1))
  plot(lz_1kb, main = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i], " (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i], " (1-kb shift)"))
  plot(lz_10kb, main = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i], " (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i], " (10-kb shift)"))
  plot(lz_custom, main = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i], " (~", win, "-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("kss_SPO11_RPI34_RPI35_rangerPeaks vs ", otherNames[i], " (~", win, "-kb shift)"))
  dev.off()
}
